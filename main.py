"""
Telegram IP Grabber Bot - Railway Ready with error logging
"""
...
if not API_ID:
    LOGGER.error("API_ID environment variable is missing")
    sys.exit(1)
if not API_HASH:
    LOGGER.error("API_HASH environment variable is missing")
    sys.exit(1)
if not BOT_TOKEN:
...
async def main():
    try:
        ...
        # Send startup notification to owner (if set)
        owner = get_owner_id()
        if owner:
            try:
        ...
        raise  # re-raise so the process exits with error
...
if __name__ == "__main__":
    try:
        ...
        LOGGER.exception("Unhandled exception in asyncio.run")
        sys.exit(1)
