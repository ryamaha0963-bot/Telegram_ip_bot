"""
Telegram IP Grabber Bot - Fixed for Railway
"""
...
OWNER_ID = int(os.getenv("OWNER_ID", 0))  # optional, can be set via /approve
...
async def approve_cmd(client, message):
    owner = get_owner_id()
    if owner == 0:
        # If no owner set, this user becomes owner
...
async def main():
    await bot.start()
    LOGGER.info("Bot started. Use /start in private chat.")
    await idle()
    await bot.stop()
